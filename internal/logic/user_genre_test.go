...
func TestGetUserGenre(t *testing.T) {
	...
	var userID uint = 1
	uri := fmt.Sprintf("/api/v1/user/genres/%d", userID)

	daoMock.EXPECT().FindUserGenres(gomock.Any(), userID).Times(1).Return(&[]models.GenreInfo{
		{
			GenreId: 1,
		},
		{
...
func TestUpdateUserGenre(t *testing.T) {
	ctl := gomock.NewController(t)
	defer ctl.Finish()
	...
	daoMock.EXPECT().UpdateUserGenres(gomock.Any(), gomock.Any(), userID).Times(1).Return(nil)

	r := httptest.NewRequest("PATCH", uri, bytes.NewBuffer(genresID))
